...
def cite_item(item, dataset, scorer, top_k, temperature, threshold, text_field="answer"):
    ...
    if scorer == "keyword_jaccard":
        raise ValueError("keyword_jaccard needs the extractor; not available CPU-only")
...
def build(dataset, scorer, top_k=5, temperature=0.05, threshold=0.0,
          limit=None, retriever="gtr", text_field="answer"):
    items = load(dataset, retriever, limit)
    data, dropped = [], 0
    for it in items:
        new = cite_item(it, dataset, scorer, top_k, temperature, threshold, text_field)
        if new is None:
            dropped += 1
            continue
        data.append(new)
